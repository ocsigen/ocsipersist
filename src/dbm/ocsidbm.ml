...
let errlog s =
  let date =
    ...
    Printf.sprintf "%02d-%02d-%04d %02d:%02d:%02d" t.Unix.tm_mday
      (t.Unix.tm_mon + 1) (1900 + t.Unix.tm_year) t.Unix.tm_hour t.Unix.tm_min
      t.Unix.tm_sec
  in
...
let find_create_table name =
  try Tableoftables.find name !tableoftables with Not_found -> open_db name

let find_dont_create_table name =
  try Tableoftables.find name !tableoftables
  with Not_found -> open_db_if_exists name

let db_get store name = find (find_dont_create_table store) name

let db_remove store name =
  try remove (find_dont_create_table store) name with
  | Not_found -> ()
  | Dbm.Dbm_error "dbm_delete" -> ()

let db_replace store name value = replace (find_create_table store) name value
